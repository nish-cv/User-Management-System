...
public class UserOperations
{
	...
	public boolean addUser(User user)
	{
		...
		String query = "insert into \"user\".user_details(contact,firstname,lastname,address,age,username,password) VALUES(?,?,?,?,?,?,?);";
		int status = 0;
		try
		{
	...
	public List<User> readUser()
	{
		...
		String query = "select * from \"user\".user_details where usertype=1";
		try
		{
			pstObject = connectionObject.prepareStatement(query);
			...
			while(result.next())
			{
				...
				User user = new User(id,age,contact,address,firstname,lastname,username,password);
				userList.add(user);
			}
		}
		catch(Exception e)
